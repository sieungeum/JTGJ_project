...
<script>
        ...
        function basicCalculator() {
            ...
            const num22 = parseFloat(document.getElementById('num22').value);
            const num3 = parseFloat(document.getElementById('num3').value);
            const num33 = parseFloat(document.getElementById('num33').value);
            const num4 = parseFloat(document.getElementById('num4').value);
            const num44 = parseFloat(document.getElementById('num44').value);
            const num5 = parseFloat(document.getElementById('num5').value);
            const num55 = parseFloat(document.getElementById('num55').value);
            const result5 = ((num1 / num11)+(num2 / num22)+(num3 / num33)+(num4 / num44)+(num5 / num55)) * 2.75;
...
<body>
	...
	<h3>탄소배출량 계산기</h3>
		<div class="main">
			...
			<div class="mini">
				...
				<input class="box" type="number" id="inputValue2" placeholder="월간 가스 사용량">
	...
	<div id="calculator2" class="calculator">
		...
		<div>
			<div>
				<label for="production">자체 생산 에너지 (kWh/년):</label> <input type="number"
					id="production" class="box2" placeholder="연간 에너지 생산량 입력">
			</div>
	
			<div>
				<label for="consumption">소비 에너지 (kWh/년):</label> <input type="number"
					id="consumption" class="box2" placeholder="연간 에너지 소비량 입력">
			</div>
		</div>
		...
		<div class="main">
			<div class="mini2">
				<div>
					...
					<input type="number" id="num1" class="box2" placeholder="연간 난방에서지 소요량">
				</div>
				<div>
					<label for="num11">난방에너지가 요구되는 바닥면적</label>
					<input type="number" id="num11" class="box2" placeholder="난방 바닥면적">
				</div>
			</div>
			<div class="mini2">
				<div>
					<label for="num2">냉방에너지 소요량</label>
					<input type="number" id="num2" class="box2" placeholder="연간 냉방에서지 소요량">
				</div>
				<div>
					<label for="num22">냉방에너지가 요구되는 바닥면적</label>
					<input type="number" id="num22" class="box2" placeholder="냉방 바닥면적">
				</div>
			</div>
			<div class="mini2">
				<div>
					<label for="num3">급탕에너지 소요량</label>
					<input type="number" id="num3" class="box2" placeholder="연간 급탕에서지 소요량">
				</div>
				<div>
					<label for="num33">급탕에너지가 요구되는 바닥면적</label>
					<input type="number" id="num33" class="box2" placeholder="급탕 바닥면적">
				</div>
			</div>
			<div class="mini2">
				<div>
					<label for="num4">조명에너지 소요량</label>
					<input type="number" id="num4" class="box2" placeholder="연간 조명에서지 소요량">
				</div>
				<div>
					<label for="num44">조명에너지가 요구되는 바닥면적</label>
					<input type="number" id="num44" class="box2" placeholder="조명 바닥면적">
				</div>
			</div>
			<div class="mini2">
				<div>
					<label for="num5">환기에너지 소요량</label>
					<input type="number" id="num5" class="box2" placeholder="연간 환기에서지 소요량">
				</div>
				<div>
					<label for="num55">환기에너지가 요구되는 바닥면적</label>
					<input type="number" id="num55" class="box2" placeholder="환기 바닥면적">
				</div>
			</div>
		</div>
		<p class="coment">(해당 계산은 예상 등급이며, 실제 측정 결과와 다를 수 있습니다.)</p>
		<p class="coment">(해당 계산은 주거용 건물 기준이며, 전기에너지를 기준으로 측정하였습니다 .)</p>
		<p class="coment">(대부분 퉁쳐서 하긴했는데 주거용 외 건물이나, 사용에너지?(가스, 전기, 지역 냉난방)도 선택 가능하게 해야하나?  .)</p>
